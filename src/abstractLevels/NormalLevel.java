...
public abstract class NormalLevel extends CoreLevel
{
	...
	public void paintComponent(Graphics g)
	{
		...
		if(mustacheEvent)
			mustache.draw(g);
		if(mustacheOn)
			g.drawImage(mustacheImage, player.getPlayerX() + player.getImageWidth()/2 - 75, player.getPlayerY() - 40, null);
		super.paintComponent(g);	
		g.dispose();
	}
	/**
	 * This method registers impacts e.g balls hitting one another, balls hitting frame or the paddle and checks if there is at least one ball still in the playground.
	 */
	@Override
	 ...
	 * @param power which power has been activated
	 */
	...
	/**
	 * Finishes  the game when the player won. Eventually unlocking next level(if it hasn't been unlocked yet).
	 */
	public void victory()
	{
			play = false;
